...
- 启动时输出实际运行命令和监听模式。
- 新增隐藏的 `sdgo completion [shell]`，用于生成 shell 补全脚本。
...
- 暂时移除未统一完成的 `sdgo gen` 对外入口，保留后续模块生成方案。
- `sdgo upgrade` 安装完成后会输出当前 `sdgo` 路径和版本，方便确认实际生效的二进制。
- `sdgo upgrade` 会先比较目标版本和当前版本，相同版本不再重复执行安装。

### 修复

- 移除了源码和文档中的本机固定模板路径。
- 优化私有模板仓库或分支/tag 不存在时的 Git clone 失败提示。
- 修复 `sdgo run` / `sdgo serve` 按 `Ctrl-C` 后可能只退出 shell、真实服务进程残留导致端口占用的问题。
- 子进程异常退出时，热重载 runner 会直接返回错误，不再继续挂起。
