...
- In a generated project directory: run and hot reload.
...
Upgrade the installed `sdgo` binary from GitHub:
...
`sdgo completion` is hidden from `sdgo --help`, but it can generate completion scripts.
...
Restart the terminal after installing the completion file.
